...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn cast_immediate(
        ...
        imm = match (from_scalar.primitive(), to_scalar.primitive()) {
            (Int(_, is_signed), Int(_, to_is_signed)) => {
                bx.intcast(imm, to_backend_ty, IntCastOp::new(is_signed, Some(to_is_signed)))
            }
            (Float(_), Float(_)) => {
            ...
            (Int(_, is_signed), Pointer(..)) => {
                let usize_imm =
                    bx.intcast(imm, bx.cx().type_isize(), IntCastOp::new(is_signed, None));
