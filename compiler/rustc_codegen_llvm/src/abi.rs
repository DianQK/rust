    ...
    fn store(
        ...
        match &self.mode {
            ...
            PassMode::Cast { cast, pad_i32: _ } => {
                ...
                let align = self.layout.align.abi;
                let dest_ptr = dst.val.llval;
                let mut index = 0;
                let mut offset = 0;
                for reg in cast.prefix.iter().filter_map(|&x| x) {
                    let from = bx.extract_value(val, index);
                    let ptr = if offset == 0 {
                        dest_ptr
                    } else {
                        bx.inbounds_ptradd(dest_ptr, bx.const_usize(offset))
                    };
                    bx.store(from, ptr, align);
                    index += 1;
                    offset += reg.size.bytes();
                }
                let (val, ptr) = if index > 0 {
                    let rest = bx.extract_value(val, index);
                    let ptr = bx.inbounds_ptradd(dest_ptr, bx.const_usize(offset));
                    (rest, ptr)
                } else {
                    (val, dest_ptr)
                };
                bx.store(val, ptr, align);
            }
            _ => {
